...
| Card tagline (as shown on grid) | "Firm, high-resilience support. Cool comfort that holds its shape." | Confirmed (product owner, 2026-09-02). "Firm" is the feel this table already confirms in the spec line; **"high-resilience" is corroborated by the confirmed layer name Enduro HR Foam**. "Cool" and "holds its shape" are owner-asserted performance claims with no test recorded here |
...
| Category | FOAMICO Luxury Mattress | Confirmed |
| Card tagline (as shown on grid) | "Responsive latex bounce. Orthopaedic support that keeps you aligned." | Confirmed (product owner, 2026-09-02). Latex is named in both of Sova's stacks — Plush Core Latex in the standard grade, Pincore Latex in Natural — but note the open item below: **whether the standard grade's band is literally latex is unresolved**, and Natural is the grade this claim is safest against. "Orthopaedic" echoes the confirmed layer name OrthoBond Foam and is owner-asserted as a claim |
...
| Foam split | **Layer 6 30% / Layer 5 30% / Layer 4 20% / Layer 3 20%** of the product's foam. The cover, the convoluted top band and the bonded base are outside this split - they are upholstery and a base, not foam a percentage is quoted of - and keep the proportions they had, so the four foam bands divide their existing combined share. | Confirmed (product owner, 2026-08-26) |
| Per-grade layer composition | **Luxury 7″ — all seven bands. Premium 6.5″ — layers 1, 2, 4, 5, 6 + base (drops layer 3). Classic 6″ and Classic 5″ — layers 1, 2, 5, 6 + base (drops layers 3 and 4).** The product owner gave one rule for "sova classic" and Sova presents two Classic heights; both take it. Natural is not part of this ladder — see the row below. | Confirmed (product owner, 2026-08-27; corroborated by `SOVA CLASSIC.png` and `SOVA PREMIUM.png`) |
| Natural composition | **Natural is a different build, not this stack thinned.** **Six bands: cover → convoluted comfort foam → purple comfort foam → perforated natural latex → rebonded chip core → bonded base.** The latex sits under the purple foam, where the standard grade carries its blue one. Read from `source/Layers/SOVA NATURAL.png`. Band count, order, type and surface are confirmed by that render; the per-band thicknesses are eyeballed placeholders until a foam split is given. | Confirmed (product owner, 2026-08-31 — go by the filename, disregard the woven tag) |
| Natural per-layer names | **1 AirKnit Fabric → 2 AeroFlex Foam → 3 Cosmic Foam → 4 Pincore Latex → 5 OrthoBond Foam → Base Quilted Foam** | Confirmed (product owner, 2026-08-31; layer 3 given as "Cosmic Byte Foam" and corrected to Cosmic Foam by the owner the same day) |
...
| Card tagline (as shown on grid) | "Pocket springs that isolate motion. Balanced comfort, every night." | Confirmed (product owner, 2026-09-02). The pocket-spring core is Confirmed above and is the strongest-supported claim in this set; **motion isolation is owner-asserted** — it is the property individually pocketed springs are generally sold on, and this table records no test of it |
...
| Category | FOAMICO Luxury Mattress | Confirmed |
| Card tagline (as shown on grid) | "Cloud-like softness above. Quiet, aligned support underneath." | Confirmed (product owner, 2026-09-02). "Cloud-like" echoes the confirmed layer name Cloud Sense Foam; "quiet" is consistent with Coil layer: None — an all-foam stack has no spring unit to make a noise. "Aligned support" is owner-asserted |
...
| Coil layer | None | Confirmed |
| Variants | Classic 6″, Classic 5″, Premium 6″, Luxury 7″, Natural 6″ | Confirmed — **Luxury corrected from 6.5″ to 7″ on 2026-08-31 by the product owner**, which also makes Ultima's Luxury the same height as Resto's and Sova's. **Natural corrected from 7″ to 6″ on 2026-08-26 by the product owner**; Natural is 6″ wherever it is offered |
| Foam split | **Layer 6 30% / Layer 5 30% / Layer 4 20% / Layer 3 20%** of the product's foam. The cover, the convoluted top band and the bonded base are outside this split - they are upholstery and a base, not foam a percentage is quoted of - and keep the proportions they had, so the four foam bands divide their existing combined share. | Confirmed (product owner, 2026-08-26) |
| Per-grade layer composition | **Luxury 7″ — all seven bands. Premium 6″ — layers 1, 2, 4, 5, 6 + base (drops layer 3). Classic 6″ and Classic 5″ — layers 1, 2, 5, 6 + base (drops layers 3 and 4).** Note Classic 6″ and Premium 6″ are the same height and different builds; that is intended. Natural is not part of this ladder — see the row below. | Confirmed (product owner, 2026-08-27; corroborated by `ULTIMA CLASSIC.png`). Luxury's height corrected to 7″ on 2026-08-31 — the band membership per grade is unchanged by it |
| Natural composition | **Natural is a different build, not this stack thinned.** **Six bands: cover → convoluted comfort foam → perforated natural latex → rebonded chip core → support foam → bonded base.** The blue and purple comfort foams of the standard stack are gone rather than merely thinner, and the latex takes their place. Read from `source/Layers/ULTIMA NATURAL.png`. Band count, order, type and surface are confirmed by that render; the per-band thicknesses are eyeballed placeholders until a foam split is given. | Confirmed (product owner, 2026-08-31 — go by the filename, disregard the woven tag) |
| Natural per-layer names | **1 AirKnit Fabric → 2 AeroFlex Foam → 3 Pincore Latex → 4 Cloud Sense Foam → 5 Zero G Latex → Base Quilted Foam** | Confirmed (product owner, 2026-08-31) |
...
| Category | FOAMICO Luxury Mattress | Confirmed |
| Card tagline (as shown on grid) | "Italian-engineered comfort layers. Indulgent support, every single night." | Confirmed (product owner, 2026-09-02). **"Italian-engineered" is an origin claim and is owner-asserted: what this table confirms is Italian-language trademarked layer names** — SetaComforto™, LussoMorbido™, NubeSoave™, Struttura Forte™, FondoCore Italia™ — and nothing about where the product is engineered or made. Recorded here so the claim has a source; re-confirm it before it is repeated anywhere with more weight than a card line |
...
| Variants | Classic 6″, Classic 6.5″, Premium 7″, Luxury 6″, Natural 6″, Natural 8″, **R1000 8″, R2000 9″, R3000 10″** | Confirmed — **the R ladder was corrected from 6″ / 8″ / 9″ on 2026-08-31 by the product owner.** All three rungs moved, so this replaces the ladder rather than adjusting it. R3000 at 10″ makes Riva the tallest product in the experience, level with Luma's Luxury; R1000 at 8″ is no longer the shortest FOAMICO grade. The Classic / Premium / Luxury / Natural figures are untouched by this correction |
| Variants **presented** | **R1000 8″, R2000 9″, R3000 10″, Natural 6″** — Riva is shown by its R grades plus Natural, and by nothing else. The Classic / Premium / Luxury rows above are not offered in the experience, and neither is the second Natural at 8″. Note that Natural 6″ now sits two inches below the shortest R grade rather than level with it; Natural is a separate build, not a rung of this ladder, so that is expected. | Confirmed (product owner, 2026-08-26; R heights corrected 2026-08-31) |
...
| Per-grade layer composition | **R3000 10″ — all seven bands. R2000 9″ — drops layer 2. R1000 8″ — drops layers 2 and 3.** Riva thins from the top of the comfort stack downwards rather than from the middle, which is the opposite of how Sova, Ultima and Resto thin. Natural is not part of this ladder — see the row below. | Confirmed (product owner, 2026-08-27; corroborated by `RIVA 2K.png`). **Heights restated 2026-08-31 to match the corrected ladder — which bands each grade carries is unchanged, and no render changed shape as a result** |
| Natural composition | **Natural is a different build, not this stack thinned.** **Four bands: cover → convoluted comfort foam → perforated natural latex → bonded base.** The shortest stack of any FOAMICO grade, and the latex slab is over half the mattress. Read from `source/Layers/RIVA NATURAL.png`. Band count, order, type and surface are confirmed by that render; the per-band thicknesses are eyeballed placeholders until a foam split is given. | Confirmed (product owner, 2026-08-31 — go by the filename, disregard the woven tag) |
| Natural per-layer names | **1 Bio Weave → 2 AeroFlex Foam → 3 7 Zone Latex → Base Quilted Foam** | Confirmed (product owner, 2026-08-31) |
| Per-layer names | **1 Bio Weave → 2 SetaComforto™ → 3 LussoMorbido™ → 4 NubeSoave™ → 5 Struttura Forte™ → 6 FondoCore Italia™ → Base Quilted Foam.** This is the R-grade stack: R3000 shows all six, R2000 and R1000 show it less the bands they drop. Riva's cover is the one cover in either brand that is not AirKnit Fabric, and these are the only names in either brand carrying a trademark symbol. | Confirmed (product owner, 2026-08-31, against a RIVA 3000 reference sheet) |
| Note on the reference sheet | The RIVA 3000 sheet names layer 1 **UltraLuxe Adaptive Knit Fabric** and the base **Soft Foam with Quilting**. Neither is used: layer 1 is **Bio Weave** on the product owner's explicit instruction, and the base is **Quilted Foam** under the everywhere rule. The sheet's other six names are taken as given. | Confirmed (product owner, 2026-08-31) |
| Exact per-layer thicknesses | — | TBD |
| Dimensions | — | TBD |
...
| Previous 3D presentation | A full 3D model (`src/lib/sofaModel.js`) and viewer (`src/components/SofaViewer.jsx`) were built for this product on 2026-08-26 and are **kept in the repo but no longer routed**. The photography superseded them the same day; nothing imports them, so they cost nothing in the bundle. **If it is ever re-routed it will not run as it stands** — `SofaViewer` reads `product.model.fabric`, and this product's entry has carried no `model` key since it became photo-based, so it would throw on mount. `/textures/sofa-cum-bed/fabric.png` is still shipped and is the map it wants. Noted 2026-09-01 during a review; not fixed, because fixing it would mean guessing at a `model` block nobody has asked for. | Confirmed |
...
| Layer construction | AirKnit Fabric (cover) → AeroFlex Foam → Float Sense Foam → OrthoBond Foam → base transition/fabric layer | Confirmed — material names updated 2026-08-31; the Float Sense band added the same day, see the two rows below |
| Excluded layer — **SUPERSEDED 2026-08-31** | This entry used to read: *a green foam layer appears between AeroFlex Foam and OrthoBond Foam in one reference image (`DURO_PREMIUM.png`) — this layer does not exist in the real product and must never be rendered.* **That was wrong, and the reason it was wrong is worth keeping.** It assumed every Duro grade shares one stack, so two renders showing different band counts had to mean one of them was faulty. They were not in conflict — they are different grades. **The green band is real: it is Float Sense Foam, the same material Maxa and Magic carry.** Classic and Premium have it; Luxury does not. There is no excluded layer in Duro. | Confirmed (product owner, 2026-08-31) — explicit override of a previously Confirmed entry |
| Coil layer | None | Confirmed |
| Variants | Classic 5″, Premium 6″, Luxury 6″ | Confirmed |
| Per-grade layer composition | **Classic 5″ and Premium 6″ — all five bands. Luxury 6″ — layers 1, 2, 4 + base (drops layer 3, Float Sense Foam).** Duro is the one product whose *top* grade carries fewer bands than the grades below it; that is what the three renders show and it is intended. Premium 6″ and Luxury 6″ are the same height and different builds, as Ultima's Classic 6″ and Premium 6″ are. | Confirmed (product owner, 2026-08-31; corroborated by `DURO CLASSIC.png`, `DURO PREMIUM.png` and `DURO LUXURY.png`) |
| Foam split | **OrthoBond is 100% of Duro's support foam.** AeroFlex stays as it is, above it - explicitly confirmed by the product owner (2026-08-26) when the split was given. Still holds after Float Sense was added on 2026-08-31: Float Sense is comfort foam, not support foam, so it does not divide the share this split governs. | Confirmed |
| Per-layer names | **1 AirKnit Fabric → 2 AeroFlex Foam → 3 Float Sense Foam → 4 OrthoBond Foam → Base Quilted Foam**. Numbering follows position in the full stack, so **OrthoBond moved from Layer 3 to Layer 4** on 2026-08-31 when Float Sense was added; nothing on screen changed, because these bands carry real names and no label ever showed a number. **This retires "Super Plush LuxeKnit Fabric"**, the name previously Confirmed for Duro's cover and the only signed-off material name this project carried before 2026-08-31; the product owner replaced it with the AirKnit Fabric that every product except Riva now shares. Layer 3's spelling also moves from Ortho Bond to OrthoBond; layer 2 keeps the AeroFlex it already had | Confirmed (product owner, 2026-08-31) |
| Exact per-layer thicknesses | — | TBD |
| Dimensions | — | TBD |
...
- **The card taglines are this project's first marketing copy, and they are recorded per product above rather than only in code.** Five products carry one (Resto, Sova, Luma, Ultima, Riva); Sofa cum Bed and all three VedaSleep products carry none, and a card without one shows its name alone — an absent tagline is a product nobody has written one for, never a licence to write one. Supplied verbatim by the product owner on 2026-09-02 and shown exactly as given; `guidelines/DO_NOT_CHANGE.md` governs them like any other product claim, so **do not reword, shorten or extend one to fit a layout** — change the layout. Each entry's Status column separates what the construction here already corroborates (Resto's high resilience, Luma's pocket springs, Ultima's quiet all-foam stack) from what is owner-asserted and untested in this table (cooling, shape retention, motion isolation, orthopaedic support, Italian engineering). That distinction matters the moment this copy is reused anywhere heavier than a grid card — a spec sheet, a claim on packaging, an advertisement — and it is why the split is written down rather than left to be re-derived.
...
- **The cloth on the underside is `Anti Skid Fabric` on every mattress except Riva** (product owner, 2026-09-01). Riva is excluded and **no replacement name was given**, so its Bottom view shows no label rather than a guess; Riva is also the one product whose base is a navy denim-look cloth rather than the charcoal weave the rest carry. **This is not a layer and does not compete with one.** The Layers view names the bottom *band* Quilted Foam - transition foam bonded to a fabric-wrapped base, merged into a single band - and this names the cloth wrapping that base's underside. Both are true of the same slab seen from different sides. The name lives in `src/data/bottomFabric.js`, once, with Riva's exception beside it.
- **A grade may omit bands wherever the product's entry above records it.** Resto, Sova, Luma, Ultima and Riva all do, confirmed by the product owner on 2026-08-27 and corroborated by the per-grade render set. **Duro joined them on 2026-08-31**, and is the one product whose top grade carries *fewer* bands than the grades below it.
- **Two renders showing different band counts is not evidence that one of them is faulty.** It was read that way once - Duro's green band was recorded as an artefact that must never be rendered, on the assumption that every grade shares one stack - and it cost this project a real layer for weeks. Where renders disagree, per-grade composition is the first explanation to test, not the last.
- **A grade may also be a different build entirely, not a subset.** **Natural is**, for all three products that offer it: it carries a perforated natural-latex slab no other grade has, in place of comfort foam, so there is no set of omitted bands that could describe it. Such a grade declares its own `layers` in the product data rather than an `omitLayers` list. Natural's band count, order, type and surface come from `source/Layers/<PRODUCT> NATURAL.png` and are Confirmed; its thicknesses are placeholders until a foam split is given.
- **The Natural layer names corroborate the Natural stacks.** Supplied on 2026-08-31, after the stacks had been read off the renders, each name list came out exactly as long as the band count read from its own render - five layers plus a base for Sova and Ultima, three plus a base for Riva - and every material that also appears in the product's standard grade kept the same name there: Cloud Sense and Zero G on Ultima, OrthoBond on Sova. **`Pincore Latex` also names the pin-perforations the renders show**, which is what that slab is. Two independent sources agreeing is the strongest confirmation this table has for any Natural grade.
- **Sova Natural's layer 3 is `Cosmic Foam`, the same name it carries in Sova's standard grade.** It was first given as "Cosmic Byte Foam" and corrected by the product owner on 2026-08-31. There is no material called Cosmic Byte; do not reintroduce it from the earlier changelog entries, which record it only as what was superseded.
- **The pin-perforations of `Pincore Latex` and `7 Zone Latex` are deliberately not rendered.** The renderer's surface vocabulary has no perforated option and the product owner accepted the gap on 2026-08-31 rather than have one added. This is a closed decision, not an outstanding task - the latex bands draw plain on purpose.
- **The woven tag in the Natural render set is not evidence of which product it is.** Two of the three renders carry an "ULTIMA NATURAL" tag, including the one named for Sova, and the three stacks are plainly different products. The product owner's instruction (2026-08-31) is to go by the filename and disregard the tag. Do not re-derive a mapping from the tags later and quietly "correct" this.
