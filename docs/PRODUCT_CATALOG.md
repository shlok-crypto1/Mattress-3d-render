...
| Per-grade layer composition | **OPEN — not implemented.** The product owner's rule (2026-08-27) is: Classic and Premium carry the same bands and differ only in layer 5, which grows considerably in Premium; Luxury adds "layer 3" and its layer 5 matches Premium's. Which band "layer 3" names is unresolved. `LUMA LUXURY.png` shows **eight** bands — a blue comfort band third from the top that `LUMA CLASSIC.png` and `LUMA PREMIUM.png` do not have — while this repository's Luma stack has only seven and no blue band at all. So "layer 3" is either the blue band the renders show and the code lacks, or the near-white band the code numbers 3, which the renders show present at every grade. Until the product owner settles it, **Luma shows every band at every grade, unchanged.** | Open question |
| Exact per-layer names/thicknesses | — | TBD |
| Dimensions | — | TBD |
...
- **A grade may omit bands wherever the product's entry above records it.** Resto, Sova, Ultima and Riva all do, confirmed by the product owner on 2026-08-27 and corroborated by the per-grade render set. Luma's is an open question recorded against it; Natural's is still to come for all three products that offer it, and every Natural grade shows its whole stack meanwhile.
