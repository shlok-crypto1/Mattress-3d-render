...
- **Other products very likely have per-grade compositions too — none are confirmed yet.** The per-grade render set supplied on 2026-08-27 shows, for example, `SOVA CLASSIC.png` with five bands against `SOVA LUXURY.png`'s seven. That is a reading of reference art, not a statement from the product owner, so **nothing outside Resto has been implemented**: every other product still shows its whole stack at every grade. Do not derive a composition for them from the renders alone — wait for the foam details.
