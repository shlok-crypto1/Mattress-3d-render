...
| Natural per-layer names | **1 AirKnit Fabric → 2 AeroFlex Foam → 3 Cosmic Byte Foam → 4 Pincore Latex → 5 OrthoBond Foam → Base Quilted Foam** | Confirmed (product owner, 2026-08-31) |
...
- **`Cosmic Byte Foam` (Sova Natural, layer 3) is recorded exactly as given and is flagged.** The same purple comfort foam is `Cosmic Foam` in Sova's standard grade, so this is either a second material or a slip for the established name. Do not silently reconcile the two - it needs the product owner.
