...
The repository root holds three different kinds of thing, and telling them apart
matters more here than in most projects because two of them look alike:

| At the root | What it is | Safe to delete? |
|---|---|---|
| `index.html`, `assets/`, `brand/`, `textures/`, `products/`, `chatbot/`, `.nojekyll` | **The published site.** GitHub Pages deploys from the repository root, so this is the live deployment, byte-for-byte the contents of `app/dist/`. Regenerated by `npm run build` and synced up a level. | **No** — deleting any of it takes the live site down until the next build is synced |
| `app/` | The source code. `app/public/` is what the build copies into the published set above. | No |
| `information bot/` | **Source material for the chat bot**, and the authored file — see the chat bot entry below. Not served: the copy the site loads is generated from it into `app/public/chatbot/`. | Only after confirming the bot is no longer wanted |
| `source/` | **Source material, never shipped.** Reference photography, the cutaway render set, supplied logos. Nothing here is served or read at runtime. | Only after confirming nothing derives from it |

**`brand/` and `textures/` at the root are the deployment, not source material.**
They are byte-identical to `app/public/brand/` and `app/public/textures/` because
they *are* those folders, copied up by the publish step. This entry used to list
them as source, which was wrong and is the kind of wrong that gets a live site
deleted by someone tidying up; corrected 2026-09-01, when `source/` was created
to hold the material that genuinely is source.

When a source asset is brought into the experience it must be processed into
`app/public/` and recorded here.

### Chat bot — `information bot/information bot.html`

The Mattress Guide, added 2026-09-01. A single self-contained document: its own
CSS, its own script, its logos as base64, and no network calls of any kind. The
only external references in it are the two Google Fonts links it shares with the
site. It answers from a price and specification table written into the file — it
calls no service and has no key, so nothing about it needs configuring to run.

**This file is the source of record. Edit it, then run `npm run sync:bot`.**

`app/scripts/build-chatbot.mjs` reads it and writes `app/public/chatbot/index.html`,
which is what the site actually loads, and which the build then copies to the root
`chatbot/` with everything else in `app/public/`. The script also runs as `prebuild`,
so `npm run build` can never ship a stale copy.

The generated file is committed rather than ignored, because `npm run dev` does not
run `prebuild` and a fresh clone would otherwise serve an empty frame in development.
**Do not edit `app/public/chatbot/index.html` or the root `chatbot/index.html`** —
both are overwritten on the next build.

The one transformation the script applies is a document wrapper: the authored file
opens straight at `<title>` with no doctype, and without one the browser uses quirks
mode, where the bot's `height:100dvh` body layout does not hold. Everything else is
...
It sits beside the two brand folders in `source/` rather than inside either
because the set spans both brands and is one coherent thing:
...
`source/Foamico mattresses/Natural/` folder, whose three files were byte-identical
...
| `vedasleep-logo.png` | VedaSleep lotus + wordmark lockup for light surfaces, 1156×449 RGBA | `source/company logos/veda sleep/VEDASLEEP - LOGO.jpg.jpeg` |
...
Processed from the full-resolution JPEGs in `source/Foamico mattresses/Sofa cum Bed/`;
